...
export const getActivePredictions = (): Prediction[] => {
  return allPredictions.filter((prediction) => prediction.status === "active" && !prediction.isResolved)
}

export const getPredictionBySlug = (slug: string): Prediction | null => {
  return allPredictions.find((prediction) => prediction.slug === slug) || null
}
